...
# Четвёртый модуль. Django. Курсовая работа №4
...
Клонирование всего репозитория (Git Bash)

git clone https://github.com/пользователь/имя_репозитория.giе

cd имя_репозитория

git config remote.origin.fetch "+refs/head/*:refs/remotes/origin/*"

git fetch origin

Для каждой ветки: 
git checkout -b путь(если есть)/имя_локальной_ветки origin/путь(если есть/имя_удаленной_ветки

git checkout main

git branch -a

Можно открывать проект
